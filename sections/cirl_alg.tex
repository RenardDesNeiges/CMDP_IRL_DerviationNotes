...
\begin{remark}[Optimal Rate/Soft Policy Iteration]
    \label{rem:SPI}
...
\begin{claim}[SPI implements the Soft Bellman Operator]
  ...
  \small
    ...
    \label{alg:npg-cirl}
      ...
      \ForEach{iteration $k = 0,2,...,K-1$}{
            ...
            \bigr) $ \\
            $\bm{\lambda}^{(k+1)} \leftarrow \Pi_{\bm{\lambda} \in \mathcal{B}} \big(\bm{\lambda}^{(k)} - \eta_{z} \nabla_{\bm{\lambda}} f(\bm{w}^{(k)},\bm{\lambda}^{(k)},\bm{\theta}^{(k)}) \big) $ \\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/nice_plot_TTSA.png}
